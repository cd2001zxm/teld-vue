            ...
            <div class="widget radius-bordered" style="margin-bottom: 1px;min-height:500px">
                <div class="widget-body">
                    <div id="searchCondition" class="table-toolbar" style="padding-top:5px; padding-bottom:5px;">
                        <input type="hidden" id="DispatchFlag" value="App_PowerDispatchingShow"/>
                        <!--查询区域-->
                        <Dispatch_TableListSearch></Dispatch_TableListSearch>
                        <!--按钮组,这是权限赋值过来的-->
                        <div id="div_Button" class="table-toolbar">
                        </div>
                    </div>
                    <hr class="wide hidden-print hidden-lg hidden-md hidden-sm" style="margin-top:2px;margin-bottom:2px;" />
...
<script>
    ...
    export default {
        ...
        mounted() {
            vue_this = this;
            $(".sidebar-toggler").click();//取消左侧菜单栏
            $("#page-breadcrumbs").remove();//去掉导航栏
            $("#page-header").remove();//去掉导航栏
            $("#navbar").remove();//去掉导航栏
            ...
            $("#page-body").resize(function () {
                ...
                TPSSearch.PageBodyResizeForApp();
            });
        ...
        methods: {
            ...
            LoadPageData: function () {
                //$(".widget-buttons.buttons-bordered").css("border-left","0px none");
                //$("#div_Button").removeClass("buttons-bordered");
                ...
                TPSSearch.Dispatch.PageFlag = "App_PowerDispatchingShow";

                $("#div_Button").html($("#hidBtnHtml").val());
                ...
                require.ensure([],
                    function (require) {
                        ...
                        $("#PowerStationDiv").helper17({
                            ...
                            confirmCallback: function (helpetr) {
                                ...
                                if (curStaId == "") {//说明选择的全部电站,清空选择的cookie值
                                    ...
                                    localStorage.setItem("tps_cursta_" + Dispatch.CustomerID, curStaId);//当前选中的电站ID
                                    localStorage.setItem("tps_cursta_name_" + Dispatch.CustomerID, curStaName);//当前选中的电站名称
                                }
                        ...
                        //if (curSelSta != "undefined" && curSelSta != null && curSelStaName != null) {
                        //判断用户现在是否有cookie里的电站权限，如果有，则默认加载这个电站的集控器
                        //}
                        //绑定Soc过滤事件
